...
Cordy.prototype = _.create(EventEmitter.prototype, {
  constructor: Cordy,
  /**
   * Builds cache.
   *
      ...
      return mkdirpAsync(options.dest)
        ...
        .then(function () {
          ...
          return Promise.mapSeries(tasks, function (task) {
            ...
            var nightmareOptions = task.nightmare;
            var dest = task.dest;
   ...
   * @return {Promise}
   */
  getHTML: function (src, nightmareOptions) {
    if (_.isNil(src)) {
   ...
   * @return {Promise}
   */
  saveHTML: function (src, nightmareOptions, baseURL, dest) {
    if (_.isNil(src)) {
   ...
   * @return {Promise}
   */
  runTasks: function (tasks, options) {
    var self = this;
   ...
   * @return {String}
   */
  buildFilenameFromURI: function (uri, extension) {
   ...
   * @return {String}
   */
  relToAbs: function (data, baseURL) {
   ...
   * @return {Promise}
   */
  globStat: function (pattern, options) {
   ...
   * @return {Promise}
   */
  buildSitemap: function (globPattern, globOptions, sitemapOptions) {
    var defaultSitemapOptions = {
   ...
   * @return {Promise}
   */
  saveSitemap: function (globPattern, globOptions, sitemapOptions, dest) {
    if (_.isNil(dest)) {
      dest = 'sitemap.xml';
    }
